...
class get_Pkz(Profiles):
    ...
    def __init__(
        ...
        # Optional low-pass filter on 1-halo matter power at low k (suppresses 1h term k→0)
        if not hasattr(self, 'lowpass_Pmm1h_lowk'):
            self.lowpass_Pmm1h_lowk = halo_params_dict.get('lowpass_Pmm1h_lowk', False)
            self.kthresh_lowpass_Pmm1h_lowk = halo_params_dict.get('kthresh_lowpass_Pmm1h_lowk', 0.01)
